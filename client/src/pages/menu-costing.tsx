            ...
            <>
              <Card>
                ...
                <CardContent>
                  ...
                  {(() => {
                    const outputVat = selectedMenuItem.outputVatRate || 20;
                    const priceIncVat = selectedMenuItem.vatIncluded
                      ? selectedMenuItem.sellingPrice
                      : selectedMenuItem.sellingPrice * (1 + outputVat / 100);
                    const priceExVat = selectedMenuItem.vatIncluded
                      ? selectedMenuItem.sellingPrice / (1 + outputVat / 100)
                      : selectedMenuItem.sellingPrice;
                    const vatAmount = priceIncVat - priceExVat;
                    const inputVatOnIngredients = recipeIngredients.reduce((sum, ri) => {
                      const ing = allIngredients.find((x) => x.id === ri.ingredientId);
                      if (!ing) return sum;
                      const ingVat = ing.vatRate || 20;
                      const ingPrice = ri.quantity * ing.currentPrice;
                      const ingVatAmt = ing.vatIncluded ? ingPrice - ingPrice / (1 + ingVat / 100) : ingPrice * ingVat / 100;
                      return sum + ingVatAmt;
                    }, 0);
                    const netVatPayable = vatAmount - inputVatOnIngredients;
                    return (
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 p-3 rounded-md border border-dashed">
                        <div className="text-center">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Price ex-VAT</span>
                          <div className="text-sm font-semibold" data-testid="text-price-ex-vat">£{priceExVat.toFixed(2)}</div>
                        </div>
                        <div className="text-center">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Output VAT ({outputVat}%)</span>
                          <div className="text-sm font-semibold" data-testid="text-output-vat">£{vatAmount.toFixed(2)}</div>
                        </div>
                        <div className="text-center">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Input VAT (ingredients)</span>
                          <div className="text-sm font-semibold text-emerald-600 dark:text-emerald-400" data-testid="text-input-vat">-£{inputVatOnIngredients.toFixed(2)}</div>
                        </div>
                        <div className="text-center">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Net VAT Payable</span>
                          <div className={`text-sm font-semibold ${netVatPayable >= 0 ? "" : "text-emerald-600 dark:text-emerald-400"}`} data-testid="text-net-vat">£{netVatPayable.toFixed(2)}</div>
                        </div>
                      </div>
                    );
                  })()}
