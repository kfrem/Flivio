...
function MetricCard({
  ...
  const inner = (
    <Card className={drillHref ? "hover-elevate cursor-pointer" : ""} data-testid={`card-metric-${title.toLowerCase().replace(/\s+/g, "-")}`}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {drillHref && <ArrowRight className="h-3 w-3 text-muted-foreground" />}
          </div>
        </div>
      </CardContent>
    </Card>
  ...
  if (drillHref) {
    return <Link href={drillHref}>{inner}</Link>;
  }
  return inner;
}
...
export default function Dashboard() {
  ...
  const costBreakdownData = latestData ? [
    { name: "Food", value: latestData.foodCost, fill: CHART_COLORS[0], drillKey: "foodCost" },
    { name: "Labour", value: latestData.labourCost, fill: CHART_COLORS[1], drillKey: "labourCost" },
    { name: "Energy", value: latestData.energyCost, fill: CHART_COLORS[2], drillKey: "energyCost" },
    { name: "Rent", value: latestData.rentCost, fill: CHART_COLORS[3], drillKey: "rentCost" },
    { name: "Marketing", value: latestData.marketingCost, fill: CHART_COLORS[4], drillKey: "marketingCost" },
    { name: "Supplies", value: latestData.suppliesCost, fill: CHART_COLORS[5], drillKey: "suppliesCost" },
    { name: "Waste", value: latestData.wasteCost, fill: CHART_COLORS[6], drillKey: "wasteCost" },
    { name: "Technology", value: latestData.technologyCost, fill: CHART_COLORS[7], drillKey: "technologyCost" },
  ] : [];
    ...
    <div className="p-6 space-y-6 overflow-auto h-full" data-testid="page-dashboard">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          icon={Utensils}
          drillHref="/app/drill-down?category=foodCost&label=Food%20%26%20Ingredients"
          healthColor={
            metrics
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          icon={Users}
          drillHref="/app/drill-down?category=labourCost&label=Labour"
          healthColor={
            metrics
        ...
        <MetricCard
          ...
          subtitle="target: 5-8%"
          icon={Zap}
          drillHref="/app/drill-down?category=energyCost&label=Energy%20%26%20Utilities"
          healthColor={
            metrics
        ...
        <MetricCard
          ...
          subtitle="target: < 3%"
          icon={AlertTriangle}
          drillHref="/app/drill-down?category=wasteCost&label=Food%20Waste"
          healthColor={
            metrics
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      onClick={(data) => {
                        if (data?.drillKey) {
                          setLoc(`/app/drill-down?category=${data.drillKey}&label=${encodeURIComponent(data.name)}`);
                        }
                      }}
